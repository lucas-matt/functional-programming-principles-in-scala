...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.map { case (n, m) => (n, m :: history) }.toStream
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val newExplored = explored ++ newNeighbours.map { case(b,_) => b }
      initial #::: from(newNeighbours, newExplored)
    }
  }
